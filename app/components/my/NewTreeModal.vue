...
const handleSuccess = async (treeId: string) => {
  isOpen.value = false
  emit('changed')
...
<template>
  <UModal
    ...
    :ui="{ content: 'bg-default flex flex-col max-w-md' }"
  >
    <template #body>
      <UTabs :items="tabs">
        <template #create>
          <form class="mt-4 flex flex-col gap-4" @submit.prevent="createLogic.createTree">
            <div class="space-y-4">
              <UFormField :label="t('my.new_tree_modal.form.name_label')" required>
                <UInput
                  v-model="createLogic.treeName.value"
                  :placeholder="t('my.new_tree_modal.form.name_placeholder')"
                  size="lg" color="neutral" variant="subtle" class="w-full"
                />
              </UFormField>
              ...
              <UFormField :label="t('my.new_tree_modal.form.desc_label')">
                <UTextarea
                  ...
                  class="w-full" color="neutral" variant="subtle"
                />
              </UFormField>
...
<!--              </div>-->
            ...
            <div class="mt-4 flex justify-end gap-2">
              ...
              <UButton type="submit" :loading="createLogic.pending.value" :disabled="createLogic.isSubmitDisabled.value">
        ...
        <template #import>
          <div class="mt-4 flex flex-col gap-8">
            ...
            >
              <UIcon name="i-lucide-upload-cloud" class="size-12 text-muted mb-4" />
              <p class="text-sm font-medium text-highlighted">
                {{ importLogic.gedcomFile.value ? importLogic.gedcomFile.value.name : t('my.new_tree_modal.import.click_to_upload') }}
              </p>
              <p class="text-xs text-muted mt-1">{{ t('my.new_tree_modal.import.support_text') }}</p>
