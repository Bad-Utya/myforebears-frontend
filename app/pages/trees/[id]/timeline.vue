...
const treeId = computed(() => {
  ...
  return Array.isArray(routeId) ? routeId[0] : (routeId as string)
})
...
// TODO: is it clean to use like other use composables here
const {
  pending, events, tree, persons, personOptions,
  personNameById, eventTypeById, loadPageData,
  eventTypes, fetchEvents, removeEvent
} = useTimelineData(treeId)

const {
  undatedEvents, positionedEvents, yearMarkers, timelineYears
} = useTimelineLayout(
  events,
...
const {
  scale, translateX, isDragging,
  zoomIn, zoomOut, shift, fitTimeline,
  startDragging
...
watch([timelineYears, timelinePersonIds], () => {
  nextTick(() => fitTimeline(timelineYears.value.length))
})

const openEditEventModal = (event: EventDTO) => {
  isModalOpen.value = true
  currentEvent.value = event
}

const deleteEvent = (event: EventDTO) => {
  removeEvent(event.event_id ?? event.id ?? '')

  // TODO: neeed to check if removed successfully
...
<template>
  <UMain class="timeline-page min-h-screen p-4 sm:p-6">
    <div class="mx-auto flex max-w-375 flex-col gap-4">
      ...
      <section class="rounded-xl shadow-lg shadow-carbon-800 bg-default p-4 sm:p-6 flex flex-col gap-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p class="mt-1 text-sm text-muted">
              ...
              ? `${timelineYears[0]} – ${timelineYears.at(-1)}`
              : t('timeline.no_dated_events')
              }}
            </p>
          </div>
          <div class="flex items-center gap-2">
            <UButton
              ...
              @click="navigateTo(`/trees/${treeId}/main`)"
            >
              {{ t('common.back') }}
            </UButton>
          </div>
